package fl.controls
{
   ...
   public class TileList extends SelectableList implements IFocusManagerComponent
   {
      ...
      override protected function drawList() : void
      {
         ...
         _loc14_ = renderedItems = new Dictionary(true);
      ...
      public function get innerWidth() : Number
      {
         ...
         drawNow();
         _loc1_ = getStyleValue("contentPadding") as Number;
      ...
      override protected function moveSelectionVertically(param1:uint, param2:Boolean, param3:Boolean) : void
      {
         ...
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         _loc4_ = Math.max(1,Math.max(contentHeight,availableHeight) / _rowHeight << 0);
         _loc5_ = Math.ceil(Math.max(columnCount * rowCount,length) / _loc4_);
         _loc6_ = Math.ceil(length / _loc5_);
         switch(param1)
         {
            case Keyboard.UP:
      ...
      public function set rowCount(param1:uint) : void
      {
         ...
         _loc3_ = Math.ceil(length / param1) > width / columnWidth >> 0 && _scrollPolicy == ScrollPolicy.AUTO || _scrollPolicy == ScrollPolicy.ON;
      ...
      public function set columnCount(param1:uint) : void
      {
         ...
         __columnCount = 0;
         _loc2_ = Number(getStyleValue("contentPadding"));
         _loc3_ = Math.ceil(length / param1) > height / rowHeight >> 0 && _scrollPolicy == ScrollPolicy.AUTO || _scrollPolicy == ScrollPolicy.ON;
      ...
      override public function get rowCount() : uint
      {
         ...
         var _loc3_:uint = 0;
         _loc1_ = Number(getStyleValue("contentPadding"));
         _loc2_ = Math.max(1,(_width - 2 * _loc1_) / _columnWidth << 0);
         _loc3_ = Math.max(1,(_height - 2 * _loc1_) / _rowHeight << 0);
         if(_scrollDirection == ScrollBarDirection.HORIZONTAL)
         {
      ...
      override public function scrollToIndex(param1:int) : void
      {
         ...
         drawNow();
         _loc2_ = Math.max(1,contentWidth / _columnWidth << 0);
