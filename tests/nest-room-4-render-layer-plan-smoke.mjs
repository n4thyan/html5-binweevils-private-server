...
import {
  ...
  getNestRoom4RenderBasicsLayerSummary
} from "../src/rooms/NestRoom4RenderBasicsLayerPlan.js";

assert.equal(NEST_ROOM_4_RENDER_BASICS_LAYER_PLAN.length, 3);

const roomBG_spr = NEST_ROOM_4_RENDER_BASICS_LAYER_PLAN.find((layer) => layer.key === "roomBG_spr");
...
const door1_mc = NEST_ROOM_4_RENDER_BASICS_LAYER_PLAN.find((layer) => layer.key === "door1_mc");
...
const door2_mc = NEST_ROOM_4_RENDER_BASICS_LAYER_PLAN.find((layer) => layer.key === "door2_mc");
...
const summary = getNestRoom4RenderBasicsLayerSummary();
assert.equal(summary.layerCount, 3);
assert.equal(summary.baseLayer, "roomBG_spr");

console.log("nestRoom4 render layer plan smoke test passed");
console.log(
  NEST_ROOM_4_RENDER_BASICS_LAYER_PLAN
