...
const roomName = process.argv[2] || "nestRoom1";
const inputPath = process.argv[3] || `reference/rooms/nest-dump/SWF XML/${roomName}.xml`;
const outputPath = process.argv[4] || `public/generated/${roomName}-xml-scene.json`;
const exportRoot = `reference/rooms/nest-dump/${roomName}.swf`;
...
const payload = {
  generatedBy: "scripts/build-nest-room-xml-scene-json.mjs",
  roomName,
  ...
  mappingStrategy: [
    ...
    "simple DefineSprite_N folders supported for Nest"
  ],
  stats: {
...
console.log("Nest room XML scene JSON built");
...
console.log(`next: open /probes/${roomName}-render-basics.html`);
...
async function listSpriteAssets(spritesRoot) {
  ...
  for (const entry of entries) {
    ...
    const classTail = classSlug ? classSlug.split(".").at(-1) : "";
